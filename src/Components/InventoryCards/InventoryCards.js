...
const InventoryCards = (inventories) => {
  ...
  return inventories.map((inventory) => (
    <section className="inventory-cards-wrap" key={inventory.id}>
      <div className="inventory-cards__warehouse-name-wrap">
        <div className="inventory-cards__warehouse-name-title">
          INVENTORY ITEM
        </div>
        <Link to={`/inventories/${inventory.id}`}>
          <p className="inventory-cards__warehouse-name">
            {inventory.item_name}{" "}
            <img
              src={chevronIcon}
              alt="chevron icon right"
              className="inventory-cards_warehouse-name-icon"
            />
          </p>
        </Link>
      </div>
